...
class WarningDoesNotExists(tk.Toplevel):
    ...
    def initUI(self):
        ...
        download_button = ttk.Button(buttons_frame, text=f"Download {self.program}", command=self.open_browser)
        download_button.pack(side='left', expand=True, pady=10, padx=10, )
...
if __name__ == "__main__":
    root = tk.Tk()
    center(root)
    WarningDoesNotExists(controller=None, root=root, info='Chrome is not installed', program='Chrome')
